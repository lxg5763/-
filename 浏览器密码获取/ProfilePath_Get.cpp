...
string getProfilePath() {
	...
	if (appDataPath != NULL) {
		...
		do {
			...
			{	// dwFileAttributes是目标文件标记，&表示做与运算，FILE_ATTRIBUTE_DIRECTORY是文件夹的标志符，这句if的功能就是判断目标文件是否为文件夹
				string str = ffd.cFileName;

				if (str.find("release") != str.npos)
				{	// profiles下带有release的文件夹即为包含logins.json的文件夹，如845f4a08.default-release，安装时随机生成带有release名字的文件
